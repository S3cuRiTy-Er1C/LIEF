...
bool is_art(const std::vector<uint8_t>& raw) {
  ...
  std::copy(
    ...
    reinterpret_cast<const uint8_t*>(raw.data()) + sizeof(ART::magic),
    magic);

  return std::equal(std::begin(magic), std::end(magic), std::begin(ART::magic));
}
...
art_version_t version(const std::string& file) {
  ...
  if (std::ifstream ifs{file, std::ios::in | std::ios::binary}) {
    ...
    ifs.seekg(sizeof(ART::magic), std::ios::beg);
...
art_version_t version(const std::vector<uint8_t>& raw) {
  ...
  std::copy(
    reinterpret_cast<const uint8_t*>(raw.data()) + sizeof(ART::magic),
    reinterpret_cast<const uint8_t*>(raw.data()) + sizeof(ART::magic) + sizeof(version) + 1,
    version);
